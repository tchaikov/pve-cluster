...
fn create_test_config() -> std::sync::Arc<pmxcfs_config::Config> {
    pmxcfs_config::Config::new(
        "testnode".to_string(),
        "127.0.0.1".to_string(),
        1000, // www-data gid
        false,
...
fn test_link_plugin_has_quorum_aware_mode() {
    ...
    let local_plugin = registry_quorate.get("local").expect("local symlink should exist");
    ...
    assert_eq!(local_plugin.mode(), 0o777, "Link plugin base mode should be 0o777");
